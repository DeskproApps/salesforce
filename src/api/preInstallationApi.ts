...
export const getMePreInstalled = (client: IDeskproClient, settings: Settings) =>
    preInstalledRequest(client, settings, "/services/data/v55.0/chatter/users/me", "GET")
;
...
const preInstalledRequest = async (
    ...
    if (!every([settings?.salesforce_instance_url, settings?.client_key, settings?.client_secret, settings?.global_access_token])) {
        throw new Error("Client key, secret, instance URL and global access tokens are not defined");
    }
